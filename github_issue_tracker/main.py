...
@app.get('/all_issues',status_code=status.HTTP_200_OK)
async def get_all_issues(db:db_dependency):
    return db.query(Issues).all()


@app.get('/issue/{issue_id}',status_code=status.HTTP_200_OK)
...
@app.post('/addissue',status_code=status.HTTP_201_CREATED)
...
@app.put('/updateissue/{issue_id}',status_code=status.HTTP_200_OK)
...
@app.delete('/deleteissue/{issue_id}',status_code=status.HTTP_204_NO_CONTENT)
